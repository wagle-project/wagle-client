...
export default function InfoPage() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full max-w-[430px] mx-auto bg-[#0a0b1e] font-sans text-white">
      ...
      {/* ── 1. 상단 헤더 (완벽한 좌/중/우 정렬로 겹침 해결) ── */}
      <header className="flex items-center justify-between px-5 h-20 shrink-0 bg-[#0a0b1e]">
        {/* 왼쪽: 뒤로가기 버튼 */}
        <button
          onClick={() => router.back()}
          className="w-12 h-12 flex items-center justify-start text-[#ff3d71] bg-transparent outline-none transition-transform active:scale-95"
          aria-label="뒤로가기"
        >
          {/* 뒤로가기 화살표 <- */}
        ...
        {/* 중앙: 타이틀 (About 기본 + WagleWagle 로고폰트) */}
      ...
      <main className="flex-1 overflow-y-auto px-6 pb-32 scrollbar-hide">
          ...
          <div className="flex flex-col gap-[24px]">
            <div className="flex items-center gap-6 p-5 rounded-2xl bg-white/5">
              ...
              <div className="flex-1">
                <h4 className="text-[15px] font-bold mb-2">인파 밀집 사고 예방</h4>
                <p className="text-[12px] text-white/50 leading-[1.8] break-keep">대학 축제, 빵 축제 등 대형 이벤트의 과밀집 위험을 실시간으로 감지합니다.</p>
              </div>
            </div>
            <div className="flex items-center gap-6 p-5 rounded-2xl bg-white/5">
              <div className="w-12 h-12 shrink-0 flex items-center justify-center">
              ...
              <div className="flex-1">
                <h4 className="text-[15px] font-bold mb-2">불편한 종이 팜플렛</h4>
                <p className="text-[12px] text-white/50 leading-[1.8] break-keep">더 이상 젖고 찢어지는 종이 지도는 필요 없습니다. 스마트폰 하나면 충분합니다.</p>
              </div>
            </div>
            <div className="flex items-center gap-6 p-5 rounded-2xl bg-white/5">
              <div className="w-12 h-12 shrink-0 flex items-center justify-center">
              ...
              <div className="flex-1">
                <h4 className="text-[15px] font-bold mb-2">응급 상황 및 시설 찾기</h4>
                <p className="text-[12px] text-white/50 leading-[1.8] break-keep">가장 가까운 화장실, 의무실, 비상 출구를 단 몇 초만에 확인하세요.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        {/* Meet Our Wagle (간격 대폭 확대) */}
        <section className="mb-[60px] flex flex-col items-center text-center">
          {/* 하얀색 라인으로 둥근 라벨 */}
          <div className="px-8 py-3 rounded-full border-2 border-white/20 mb-10 bg-transparent">
            <span className="font-bold text-base tracking-wide text-white">Meet Our Wagle</span>
          </div>
          
          <p className="text-[12px] text-white/50 mb-12 leading-[2] break-keep max-w-[85%]">
